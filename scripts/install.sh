#!/usr/bin/env bash
# nexo installer: downloads the right prebuilt binary from the latest GitHub
...
#   NEXO_INSTALL_DIR  target directory (default: /usr/local/bin if writable,
...
main() {
  ...
  case "$os" in
    ...
    mingw*|msys*|cygwin*)
      ...
      os="windows"; ext="zip"; bin="nexo.exe" ;;
  ...
  # --- download and verify --------------------------------------------------
  archive="nexo_${bare}_${os}_${arch}.${ext}"
  base="https://github.com/$REPO/releases/download/$version"
  tmp=$(mktemp -d)
  trap 'rm -rf "$tmp"' EXIT

  say "downloading nexo $version ($os/$arch)..."
